...
#REPO_URL = 'https://github.com/mengmomengshang/TSDT.git' #(1)
REPO_URL = 'git@github.com:mengmomengshang/TSDT.git' #(1)
...
def _get_latest_source(source_folder):
    if exists(source_folder + '/.git'): #(1)
        ...
        #run(f'cd {source_folder} && git fetch --all')
        #run(f'cd {source_folder} && git reset --hard origin/master')

    else:
        run(f'git clone {REPO_URL} {source_folder}') #(4)
    current_commit = local("git log -n 1 --format=%H", capture=True) #(5)
    #current_commit = run(f'cd {source_folder} && git log -n 1 --format=%H')
